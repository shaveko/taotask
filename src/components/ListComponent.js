...
export default class ListComponent extends React.Component {
  ...
  fetchingDone = items => {
    this.setState(prevState => {
      ...
      let columnConfig = setupColumnConfig({
  ...
  loadMore = () => {
    ...
    this.props.dataProvider
      .loadItems(this.state.offset, this.state.limit)
      .then(this.fetchingDone)
      .catch(e => this.setState({ errorMessage: e.message }));
  };
  ...
  render() {
    return (
      <div className="ListComponent_listContainer">
        <header className="App-header">
          <p id="headerText">{this.props.header}</p>
        </header>
        <ItemsTable
        ...
        <div role="alert" className="ListComponent_errorBox">
...
function setupColumnConfig(props) {
  ...
  if (!props.columnConfig) {
    if (props.items && props.items.length) {
      columnConfig = Object.keys(props.items[0]).map(key => {
        return {
          key: key,
          label: key
        };
